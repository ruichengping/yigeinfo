...
layui.use(['form','element'],function () {
    ...
    //监听提交按钮
    form.on('submit(addThirdParty)', function(){
        $.ajax({
            type:"post",
            url:"/thirdParty/addthirdParty.json",
            data:$(".form-thirdParty").serialize()
        }).done(function (data) {
            if(data.success){
                layer.confirm('新增成功', {
                    title:"信息",
                    ...
                    window.location.href="/thirdParty/thirdPartyList.html";
                }, function(index){
                    layer.close(index);
